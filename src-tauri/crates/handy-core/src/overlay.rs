...
fn get_monitor_with_cursor(app_handle: &AppHandle) -> Option<tauri::Monitor> {
    if let Some(mouse_location) =
        crate::focus_target::monitor_center().or_else(|| input::get_cursor_position(app_handle))
    {
...
fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    if state == "recording" || state == "streaming" {
        crate::status::preparing(app_handle, state == "streaming");
    } else {
        crate::status::phase(app_handle, state);
    }
...
fn hide_delay(phase: &str, has_text: bool) -> Option<u64> {
    match phase {
        "error" if has_text => None,
        "error" => Some(3500),
        "completed" => Some(750),
        _ => Some(300),
    }
...
#[test]
fn failed_paste_keeps_a_recoverable_result_until_copy_or_dismissal() {
    assert_eq!(hide_delay("error", true), None);
    assert_eq!(hide_delay("error", false), Some(3500));
    assert_eq!(hide_delay("completed", true), Some(750));
    assert_eq!(hide_delay("cancelled", true), Some(300));
}
...
pub fn hide_recording_overlay(app_handle: &AppHandle) {
    ...
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        let delay = match hide_delay(&status.phase, !status.text.is_empty()) {
            Some(delay) => delay,
            None if !OVERLAY_ENABLED.load(Ordering::Relaxed) => 300,
            None => return,
        };
