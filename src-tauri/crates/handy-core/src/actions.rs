...
impl ShortcutAction for TranscribeAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        crate::focus_target::capture();
        crate::status::begin(app);
                    ...
                    match transcription_result {
                        Ok(transcription) => {
                            ...
                            if processed.final_text.is_empty() {
                                crate::status::result(&ah, String::new(), None);
                                utils::hide_recording_overlay(&ah);
                                set_tray_state(&ah, TrayIconState::Idle);
                            } else {
                                ...
                                ah.run_on_main_thread(move || {
                                    ...
                                    let text_for_status = final_text.clone();
                                    let paste_result = (if matches!(
                                        crate::settings::get_settings(&ah_clone).paste_method,
                                        crate::settings::PasteMethod::None
                                    ) {
                                        Ok(())
                                    } else {
                                        crate::focus_target::restore()
                                    })
                                    .and_then(|_| utils::paste(final_text, ah_clone.clone()));
                                    crate::status::result(
                                        &ah_clone,
                                        text_for_status,
                                        paste_result.as_ref().err().map(ToString::to_string),
                                    );
                                    match paste_result {
                                        Ok(()) => debug!(
                        ...
                        Err(err) => {
                            ...
                            crate::status::result(&ah, String::new(), Some(err.to_string()));
