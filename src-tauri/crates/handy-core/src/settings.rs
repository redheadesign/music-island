...
impl Serialize for SecretMap {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let protected: Result<HashMap<String, String>, String> = self
            .0
            .iter()
            .map(|(provider, value)| {
                crate::secrets::protect(value).map(|value| (provider.clone(), value))
            })
            .collect();
        protected
...
impl<'de> Deserialize<'de> for SecretMap {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let protected = HashMap::<String, String>::deserialize(deserializer)?;
        let mut values = HashMap::new();
        for (provider, value) in protected {
            // Imported or undecryptable keys are omitted. Users explicitly enter a key again.
            if let Ok(secret) = crate::secrets::unprotect(&value) {
