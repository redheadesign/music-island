...
pub async fn get_available_models(
    ...
    crate::prepare_preferences(app.clone()).await?;
    Ok(app.state::<Arc<ModelManager>>().get_available_models())
}
...
pub async fn delete_model(
    ...
    if settings.selected_model == model_id {
        if let Some(manager) = app_handle.try_state::<Arc<TranscriptionManager>>() {
            manager.unload_model().map_err(|e| format!("Failed to unload model: {}", e))?;
        }
...
pub fn switch_active_model(app: &AppHandle, model_id: &str) -> Result<(), String> {
    ...
    let transcription_manager = app.try_state::<Arc<TranscriptionManager>>();
    ...
    let _loading_guard = if crate::is_enabled() && unload_timeout != ModelUnloadTimeout::Immediately {
        Some(transcription_manager.as_ref().ok_or("Dictation is not initialized")?
            .try_start_loading().ok_or("Model load already in progress")?)
    } else { None };
    ...
    // will be loaded on-demand during the next transcription.
    if !crate::is_enabled() || unload_timeout == ModelUnloadTimeout::Immediately {
    ...
    let transcription_manager = transcription_manager.ok_or("Dictation is not initialized")?;
