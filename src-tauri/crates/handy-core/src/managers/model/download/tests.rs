...
async fn http_cancel_during_stalled_body_keeps_partial() {
    ...
    let cancellation = tokio::spawn(async move {
        // Wait for the condition this test exercises. On a busy Windows runner,
        // a fixed 300 ms delay can cancel before the HTTP headers even arrive.
        tokio::time::timeout(Duration::from_secs(10), async {
            while !fs::metadata(&observed_partial).is_ok_and(|file| file.len() == 3) {
                tokio::time::sleep(Duration::from_millis(10)).await;
            }
        })
        .await
        .expect("the first response bytes must reach the partial file");
        trigger.cancel();
    });
