...
impl ModelManager {
    ...
    async fn download_http_resumable_with_events(
        ...
        let client = reqwest::Client::builder().connect_timeout(HTTP_CONNECT_TIMEOUT);
        // Local HTTP fixtures must not depend on the developer's system proxy,
        // which can buffer the deliberately unfinished response body.
        #[cfg(test)]
