...
impl ModelManager {
    ...
    fn migrate_bundled_models(&self) -> Result<()> {
        ...
        for filename in &bundled_models {
            let bundled_path = Ok::<_, tauri::Error>(
                crate::portable::resources().join(format!("resources/models/{}", filename)),
            );
    ...
    fn migrate_gigaam_to_directory(&self) -> Result<()> {
        ...
        let vocab_path = Ok::<_, tauri::Error>(
            crate::portable::resources().join("resources/models/gigaam_vocab.txt"),
        )
    ...
    pub fn cancel_all_downloads(&self) {
        for token in self.cancel_flags.lock().unwrap().values() {
            token.cancel();
        }
    }
    pub fn has_active_downloads(&self) -> bool {
        !self.cancel_flags.lock().unwrap().is_empty()
    }
}
