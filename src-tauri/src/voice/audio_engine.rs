...
impl AudioEngine {
    pub fn new(app_handle: Option<AppHandle>) -> Self {
        ...
        let (model_switch_sender, model_switch_receiver) =
            bounded::<(String, Box<dyn denoise::DenoiseModel>)>(1);
        Self {
            ...
            model_states: std::sync::Arc::new(std::sync::Mutex::new(
    ...
    #[allow(dead_code)]
    pub fn config(&self) -> &Arc<RwLock<DenoiseConfig>> {
        &self.config
    }
    #[allow(dead_code)]
    pub fn eq_config(&self) -> &Arc<RwLock<EqConfig>> {
        &self.eq_config
    }
    #[allow(dead_code)]
    pub fn eq_config_dirty(&self) -> &Arc<AtomicBool> {
        &self.eq_config_dirty
    }
    pub fn stats(&self) -> &Arc<RwLock<AudioStats>> {
        &self.stats
    }
    #[allow(dead_code)]
    pub fn bgm_running(&self) -> &Arc<AtomicBool> {
        &self.bgm_running
    }
    #[allow(dead_code)]
    pub fn bgm_gain(&self) -> &Arc<AtomicU32> {
        &self.bgm_gain
    }
    #[allow(dead_code)]
    pub fn bgm_skip_rate(&self) -> &Arc<AtomicU32> {
        &self.bgm_skip_rate
    }
    #[allow(dead_code)]
    pub fn bgm_config(&self) -> &Arc<RwLock<BgmConfig>> {
        &self.bgm_config
    }
    #[allow(dead_code)]
    pub fn bgm_sender(&self) -> &Sender<Vec<i16>> {
        &self.bgm_sender
    }
    #[allow(dead_code)]
    pub fn bgm_was_active(&self) -> &Arc<AtomicBool> {
        &self.bgm_was_active
    }
    #[allow(dead_code)]
    pub fn explode_enabled(&self) -> &Arc<AtomicBool> {
        &self.explode_enabled
    }
    #[allow(dead_code)]
    pub fn explode_state(&self) -> &Arc<ExplodeState> {
        &self.explode_state
    }
    #[allow(dead_code)]
    pub fn monitor_enabled(&self) -> &Arc<AtomicBool> {
        &self.monitor_enabled
    }
    #[allow(dead_code)]
    pub fn monitor_point(&self) -> &Arc<AtomicU32> {
        &self.monitor_point
    }
    #[allow(dead_code)]
    #[allow(dead_code)]
    ...
    pub fn update_config(&self, new_config: DenoiseConfig) {
    ...
    pub fn set_monitor_point(&self, point: MonitorPoint) {
        self.monitor_point.store(point, Ordering::Release);
    }

    #[allow(dead_code)]
    pub fn save_model_state(&self, name: &str, state: Vec<u8>) {
        if let Ok(mut states) = self.model_states.lock() {
...
fn audio_loop(
    ...
    model_name: Option<String>,
    resource_dir: Option<std::path::PathBuf>,
    saved_model_states: std::sync::Arc<
    ...
    let mut denoise = denoise::create_model(
    ...
    let capture =
        wasapi_capture::WasapiCapture::start(&input_id, Box::new(CaptureSink { prod: prod_a }))
    ...
    let render =
        wasapi_render::WasapiRender::start(&output_id, Box::new(RenderSource { cons: cons_b }))
...
fn dsp_thread(
    ...
    while running.load(Ordering::Acquire) {
        ...
        if let Ok((new_model_name, mut new_model)) = model_switch_receiver.try_recv() {
            ...
            if let Ok(states) = saved_model_states.lock() {
                if let Some(state) = states.get(&new_model_name) {
                    ...
                    debug_log(&format!(
                        "MODEL_SWITCH: restored state for {}",
                        new_model_name
                    ));
                }
            }
        ...
        if current_config.enabled {
            ...
            if frame_count % 500 == 1 {
                ...
                debug_log_dev(&format!(
                    ...
                    current_model_name,
                    current_config.strength,
                    current_config.suppress_level,
                    in_peak,
                    out_peak
                ));
            }
            ...
            if current_config.strength < 1.0 && !denoise.has_internal_strength_control() {
                for (denoised, &original) in frame.iter_mut().zip(input_frame.iter()) {
                    *denoised = original * (1.0 - current_config.strength)
        ...
        if current_config.agc_enabled {
            ...
            for sample in frame.iter_mut() {
                *sample *= current_config.mic_gain;
            }
        }
        ...
        if bgm_running.load(Ordering::Acquire) {
            ...
            let bgm_gain_linear = f32::from_bits(bgm_gain.load(Ordering::Relaxed))
                .max(0.0)
                .min(2.0);
            if !bgm_buf.is_empty() {
        ...
        if explode_enabled.load(Ordering::Acquire) {
            ...
            crate::voice::explode::process_explode_into(
                &scaled,
                &mut output,
                &explode_state,
                &mut explode_audio,
                ...
                if let Some(ref mut m_client) = monitor.client {
                    match m_client.start_stream() {
                        Ok(()) => {
                            ...
                            debug_log(&format!(
                                "Monitor: started streaming at point {}",
                                mon_point
                            ));
                        }
                        Err(e) => {
                    ...
                    debug_log(&format!(
                        "Monitor: cannot start - client is None, mon_point={}",
                        mon_point
                    ));
                }
            }
        ...
        if stats_start.elapsed().as_secs() >= 5 {
            ...
            debug_log(&format!(
                "HEALTH: frame={} dropped={} ({:.2}%)",
                frame_count, frames_dropped, drop_rate
            ));
    ...
    debug_log(&format!(
        "dsp_thread: exiting (processed {} frames, dropped {})",
